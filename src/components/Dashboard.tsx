...
export const Dashboard: React.FC<DashboardProps> = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (ventasMes) {
        ...
        const labels = ventasMes.map((row: any) => row.mes);
      ...
      setLoading(false); // Finaliza la carga una vez que todos los datos se han obtenido
    };
  ...
  return (
    <>
      ...
      <div className={`space-y-6 ${loading ? 'hidden' : ''}`}> {/* Oculta el contenido si está cargando */}
        {/* Stats Grid */}
